...
Nationally, at the same time, was the push to provide better flood protection resulting in the [National Flood Insurance Program](/research/environmental-policy).  Columbia reacted to this push{% fn %} by directing groundwater flow into these central pools then prohibiting any building within the resultant 100-year floodplain.  There's not even much on the 500 year floodplain.  This sort of basic resilience is easy when building a planned community from the ground up.  Retrofitting resilience into an existing community is much more difficult, because stuff is already in the way.  And buying land in developed communities is expensive.
...
{% footnotes %}
  {% fnbody %}
    One might call it (•_•) / ( •_•)⌐■-■ / (⌐■_■) a groundswell.
  {% endfnbody %}
{% endfootnotes %}
